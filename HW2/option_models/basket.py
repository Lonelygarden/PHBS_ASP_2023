...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        prices = forward[:,None] * np.exp(-0.5*texp*(vol*vol)[:,None] + np.sqrt(texp) * chol_m @ znorm_m)
    else:
...
def basket_price_norm_analytic(
    ...
    spot_basket = weights @ forward
    
    norm = pf.Norm(vol_basket, intr=intr, divr=divr)
    price = norm.price(strike, spot_basket, texp, cp=cp)[0]
