...
class ModelABC(abc.ABC):
    ...
    def sigma_path(self, texp):
        ...
        sigma_t = np.exp(
            self.vov * (Z_t - self.vov / 2 * tobs[:, None])
        )  # 此处是通过Z_t的cumsum来进行每步的累积的
...
class ModelBsmMC(ModelABC):
    ...
    def price(self, strike, spot, texp=1.0, cp=1):
        ...
        """
        # (1) Generate the paths of sigma_t.
        Z_1, vol_path = self.sigma_path(
            texp
        )  # normal variable Z_1, the path of sigma_t
        sigma_t = vol_path[-1, :]  # sigma_t at maturity (t=T)
        ...
        X_1 = np.random.standard_normal(
            (n_dt, self.n_path)
        )  # normal variable X_1, indepent with Z_1
        W_1 = (
            self.rho * Z_1 + np.sqrt(1 - self.rho**2) * X_1
        )  # normal variable W_1, correlated with Z_1 of rho
        # 这里有一个问题，sigma_t和S_t会差一期，导致会有浪费的sigma_t
        sigma_times_W_accumulation = np.cumsum(
            W_1 * np.sqrt(dt) * self.sigma * vol_path[:-1], axis=0
        )
        sigma_squared_accumulation = np.cumsum(
            np.square(self.sigma * vol_path[:-1]), axis=0
        )
        S_t = spot * np.exp(
            sigma_times_W_accumulation - 1 / 2 * sigma_squared_accumulation * dt
        )
...
class ModelBsmCondMC(ModelBsmMC):
    ...
    def price(self, strike, spot, texp=1.0, cp=1):
        ...
        spot_equiv = spot * np.exp(
            self.rho / self.vov * (sigma_t - self.sigma)
